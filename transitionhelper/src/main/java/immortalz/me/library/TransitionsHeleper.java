...
public class TransitionsHeleper {
    ...
    public static void startActivity(Activity activity, final Class<?> cls, View view, Integer imgId, boolean isLast) {
        startActivity(activity, cls, null, view, imgId, isLast);
    }

    private static void startActivity(final Activity activity, final Class<?> cls, Intent intent, final View view, final Integer imgId, final boolean... islast) {
        if (intent == null) {
            intent = new Intent(activity, cls);
        }
        final InfoBean bean = new InfoBean();
        ...
        view.post(new Runnable() {
            ...
            public void run() {
                ...
                if (((bean.originRect.right - bean.originRect.left) < bean.originWidth) && islast[0]) {
                    bean.originRect.right += (bean.originWidth - (bean.originRect.right - bean.originRect.left));
                } else if ((bean.originRect.right - bean.originRect.left) < bean.originWidth) {
                    bean.originRect.left -= (bean.originWidth - (bean.originRect.right - bean.originRect.left));
                } else {
                    bean.originRect.right = 0;
                }
                if (imgId == 0) {
                    bean.bitmap = createBitmap(view, bean.originWidth, bean.originHeight, false);
                } else {
        ...
        parent.post(new Runnable() {
            ...
            public void run() {
                ...
                ivTempParams.setMargins((int) (bean.originRect.left + (bean.originWidth / 2 - bean.targetWidth * bean.scalling / 2))
                        , bean.originRect.top - (parent.getTop() + bean.statusBarHeight), bean.originRect.right, 0);
