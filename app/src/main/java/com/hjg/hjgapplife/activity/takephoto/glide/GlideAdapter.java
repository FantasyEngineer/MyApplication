...
public class GlideAdapter extends RecyclerView.Adapter<GlideAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_recy_glide, parent, false);
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        switch (dataSet.get(position)) {
            case Mask: {
                int width = ScreenUtils.dp2px(context, 133.33f);
                int height = ScreenUtils.dp2px(context, 126.33f);
                Glide.with(context)
            ...
            case NinePatchMask: {
                int width = ScreenUtils.dp2px(context, 150.0f);
                int height = ScreenUtils.dp2px(context, 100.0f);
                Glide.with(context)
                        .load(R.drawable.check)
                        .apply(overrideOf(width, height))
            ...
            case CropTop:
                Glide.with(context)
                        ...
                        .apply(bitmapTransform(
                                new CropTransformation(ScreenUtils.dp2px(context, 300), ScreenUtils.dp2px(context, 100),
                                        CropType.TOP)))
                        .into(holder.image);
                break;
            case CropCenter:
